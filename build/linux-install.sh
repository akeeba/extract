#!/usr/bin/env bash
# ============================================================================
# Akeeba Extract — Linux desktop integration installer (per-user, no root)
# A cross-platform desktop application to extract Akeeba Backup archives
#
# Copyright (c) 2026 Nicholas K. Dionysopoulos / Akeeba Ltd
# License: GNU General Public License version 3, or later
#
# This script is shipped INSIDE the Linux tarball (beside the akeeba-extract
# binary). Run it after extracting the tarball to register the application and
# its icon with your desktop environment:
#
#   tar xzf Akeeba-Extract-linux-amd64.tar.gz
#   cd akeeba-extract
#   ./install.sh
#
# It installs into the per-user XDG locations (no sudo required):
#   binary  -> the extracted folder (left in place; launched from there)
#   icon    -> ~/.local/share/icons/hicolor/256x256/apps/akeeba-extract.png
#   desktop -> ~/.local/share/applications/akeeba-extract.desktop  (Exec/Icon
#              rewritten to absolute paths so the launcher works from anywhere)
#
...
APP_ID="akeeba-extract"
APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BINARY="$APP_DIR/akeeba-extract"
...
if [[ "${1:-}" == "--uninstall" ]]; then
    ...
    refresh_caches
    echo "Akeeba Extract: desktop entry and icon removed."
    exit 0
fi
...
echo "Akeeba Extract: installed desktop entry and icon for the current user."
