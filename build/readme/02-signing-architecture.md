...
This is the anchor document for how Akeeba Extract's binaries become code-signable, why
...
`Contents/MacOS/akeeba-extract`, the payload becomes
`Contents/Resources/akeeba-extract.phar`, and the whole bundle signs, notarises and
...
* **Why NOT fork `boson-php/backend-src`?** It was considered (it is Boson's
  ...
  stock-SPC build.
* **Why does the CI live on the phpmicro fork and not in Akeeba Extract?** The SFX is
  a pure function of (phpmicro source, PHP version, extension list) — nothing
  Akeeba-Extract-specific. Building it where the source lives means every push to
  `sibling-phar` refreshes the binaries for all platforms, and Akeeba Extract only
...
* **Why is signing NOT done in that CI?** Signing needs the Developer ID
  ...
  app-agnostic runtime; Akeeba Extract's packaging replaces its ad-hoc signature
  anyway.
...
3. **The split payload is not a bare phar.** Boson appends
   ...
   the phar bytes.
4. **Boson's compile cleanup chokes on the previous `Akeeba Extract.app`** (it cannot
