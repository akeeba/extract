 ...
 *  - repository    GitHub repository name   (e.g. "extract")
 *  - token         GitHub personal access token (optional, avoids rate-limiting)
 *  - outfile       Absolute path to the output file (e.g. "${dirs.dist}/akeeba-extract.json")
 */
class UpdateJson extends Task
{
    ...
    private function fetchReleases(): array
    {
        ...
        $headers = [
            ...
            'User-Agent: akeeba-extract-phing-build/1.0',
        ];
     ...
     *
     * Akeeba Extract ships a per-platform set (macOS .dmg, Windows .exe installer or .zip,
     * Linux .tar.gz) plus the cross-platform .phar; any of these marks a release as a real,
     * downloadable one.
     */
