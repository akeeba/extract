...
/*
 * Stamp the application version (Akeeba\Extract\App::VERSION) from the CHANGELOG.
 *
 ...
 * (e.g. "Akeeba Extract 0.1"). This reads that version and writes it into
 * src/App.php so the compiled binary reports it. Run by the build.xml git-*
 * targets (via `prepare`) before every compile.
 *
 ...
 * the version number out of it. An optional AKEEBA_EXTRACT_VERSION env var overrides
 * the CHANGELOG (mirroring the build scripts' fallback).
 *
...
$appFile   = $root . '/src/App.php';

$override = \getenv('AKEEBA_EXTRACT_VERSION');
$version  = ($override !== false && $override !== '') ? \trim($override) : changelogVersion($changelog);

if ($version === null || $version === '')
{
    \fwrite(\STDERR, "ERROR: could not determine a version (no AKEEBA_EXTRACT_VERSION and no readable version in {$changelog}).\n");
    exit(1);
}
