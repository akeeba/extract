 ...
 * boson.<os>.json per platform would mean multiple files kept in lock-step, so
 * instead this script filters the master boson.json down to the one requested
 * target AT RUNTIME, writes the result to a throwaway config and compiles that.
 *
 * Usage:
 ...
 * the project root so `output: ./build/output` lands in the right place — both of
 * which the Phing build.xml guarantees.
 */
...
// (mirrors build/build-all.sh). The downloaded *.sfx runtimes are left in place.
foreach (['box.json', 'entrypoint.php', 'akeeba-extract.phar'] as $stale)
{
